...
import edu.fsu.cs.mobile.benchmarks.sort.BitonicSort;

public class BitonicTask extends BenchmarkTask {
	...
	@Override protected Void doInBackground(Object... params) {
		...
		if (isNative()) {
			if (getSize() == BenchSize.SMALL)
				BitonicSort.sortSmallNative(array_list_small);
			else
				BitonicSort.sortLargeNative(array_list_large);
		} else {
			if (getSize() == BenchSize.SMALL)
				BitonicSort.sortSmall(array_list_small);
			else
				BitonicSort.sortLarge(array_list_large);
		}
		return null;
	}
