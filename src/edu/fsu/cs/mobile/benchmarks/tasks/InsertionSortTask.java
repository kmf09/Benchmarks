...
import edu.fsu.cs.mobile.benchmarks.sort.InsertionSort;

public class InsertionSortTask extends BenchmarkTask {
	...
	@Override protected Void doInBackground(Object... params) {
		...
		if (isNative()) {
			if (getSize() == BenchSize.SMALL)
				InsertionSort.sortSmallNative(array_list_small);
			else
				InsertionSort.sortLargeNative(array_list_large);
		} else {
			if (getSize() == BenchSize.SMALL)
				InsertionSort.sortSmall(array_list_small);
			else
				InsertionSort.sortLarge(array_list_large);
		}
		return null;
	}
